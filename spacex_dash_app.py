...
def get_pie_chart(site):
    ...
    if site=='ALL' :
        fig = px.pie(filtered_df,values='class', names='Launch Site', title='Percentage of success')
    else:
        filtered_df = filtered_df[filtered_df['Launch Site'] == site]
        fig = px.pie(filtered_df, names='class',  title='Percentage of success')   
    return fig
...
def scatter_plot(site,payload_range):
    ...
    filtered_df = spacex_df
    #filtered_df = spacex_df[spacex_df['Payload Mass (kg)'] >= low and spacex_df['Payload Mass (kg)'] <= high]
